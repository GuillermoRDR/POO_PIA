...
int main(int argc, char** argv){
    ...
    Model *model = new Model("models/yo/base.obj", translate, camera);
    ...
    translate = glm::vec3(100.0f, OGLobj->getTerreno()->Superficie(model->getNextTranslate()->x, model->getNextTranslate()->z), -81.0f);
    model->setTranslate(&translate);
    ...
    while (running) {
        ...
        if (renderiza) {
            ...
            if (checkCollition) { // Bandera para buscar colisiones sobre Camara/Modelo
                Model* modeloColisionado = OGLobj->lookForCollition(true);

                if (modeloColisionado != NULL && modeloColisionado->getTieneColision()) {
                    // Si hay colisión y el modelo colisionado tiene colisión habilitada
                    ...
                    // No hubo colisión, o el modelo colisionado no tiene colisión habilitada
...
bool checkInput(GameActions *actions, Scene* scene) {
    ...
    if (actions->advance != 0) {
        ...
        if (OGLobj->getTieneColision()) { // Verifica si el modelo tiene colisión habilitada
            OGLobj->setNextTranslate(&pos);
        }
        else {
            OGLobj->setTranslate(&pos); // Si no hay colisión, aplica directamente la posición
        }
        checkCollition = true;
    }
