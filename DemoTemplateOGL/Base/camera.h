...
private:
    ...
    void updateCameraVectors() {
        ...
        Right = glm::normalize(glm::cross(Front, WorldUp));  // normalize the vectors, because their length gets closer to 0 the more u look up or down which results in slower movement.
...
public:
    ...
    glm::mat4 CamaraUpdate(float pRotY, glm::vec3 *pTrans) {
        ...
        projection = glm::perspective(glm::radians(getZoom()), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 300.0f);
        // Obtenemos la vista
        view = GetViewMatrix();
